    ...
    public async Task<IIdentity> Authenticate(string authToken, CancellationToken cancellationToken)
    {
        ...
        catch (CryptographicException cryptographicException)
        {
            logger.LogWarning(
                ...
                "Cannot decrypt auth token, maybe someone is trying to forget it");
            return User.Guest;
        }

        if (!Guid.TryParse(sessionIdString, out var sessionId)) return User.Guest;
        var session = await storage.FindSession(sessionId, cancellationToken);
        if (session is null || session.ExpiresAt < DateTimeOffset.UtcNow) return User.Guest;
